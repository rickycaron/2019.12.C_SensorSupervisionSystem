...
/* Initiates a send command on the socket 'socket' and tries to send the total '*buf_size' bytes of data in 'buffer' 
 * (recall that the function might block for a while)
 * The function sets '*buf_size' to the number of bytes that were really sent, which might be less than the initial '*buf_size'
 * If a socket error happens while sending the data in 'buffer' or the connection is closed, TCP_SOCKOP_ERROR or 
 * TCP_CONNECTION_CLOSED is returned, respectively
 * If 'socket' is NULL or not yet bound, TCP_SOCKET_ERROR is returned
 */
 ...
 * If a socket error happens while receiving data or the connection is closed, TCP_SOCKOP_ERROR or TCP_CONNECTION_CLOSED 
...
//************************************** below are my own functions*********************************
tcpsock_t * tcp_sock_copy(tcpsock_t* s);
/* Copy all the contents in a tcpsock_t
 *This is only for the element_copy function for the dplist of socket_sensor_t
 */
void tcp_sock_free(tcpsock_t* s);
/* free everthing of the contents in a tcpsock_t
 *This is only for the element_copy function for the dplist of socket_sensor_t
 */
int tcp_sock_compare(tcpsock_t* s1,tcpsock_t* s2);
/* retunn 0 if not equal, 1 if equal
 *This is only for the element_copy function for the dplist of socket_sensor_t
 */


int socket_get_port(tcpsock_t * socket); 
/* Return the port number of the 'socket'
 * If 'socket' is NULL or not yet bound, TCP_SOCKET_ERROR is returned
 */
...
int socket_set_port(tcpsock_t * socket, int new_port); 
/* set the port number of the 'socket'
 * If 'socket' is NULL or not yet bound, TCP_SOCKET_ERROR is returned
 */

int socket_set_sd(tcpsock_t * socket, int new_sd); 
/* set the sd number of the 'socket'
 * If 'socket' is NULL or not yet bound, TCP_SOCKET_ERROR is returned
 */
