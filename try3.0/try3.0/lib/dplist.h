...
typedef struct dplist dplist_t; // dplist_t is a struct containing at least a head pointer to the start of the list;
// later function pointers to element_copy, element_compare, element_free, etc. will be added
...
//this one has already been implemented  
//void * dpl_get_element_at_reference( dplist_t * list, dplist_node_t * reference );
// Returns the element contained in the list node with reference 'reference' in the list. 
// If the list is empty, NULL is returned. 
// If 'reference' is NULL, the element of the last element is returned.
...
/*This will sort the list in the ascending sequency according to the element_compare()*/
dplist_t *  dpl_list_sort(dplist_t * list);

/*This will switch if not in ascending sequence*/
