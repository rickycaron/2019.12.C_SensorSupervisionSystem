...
tcpsock_t * tcp_sock_copy(tcpsock_t* s)
{
  ...
  tcpsock_t * news = (tcpsock_t *) malloc(sizeof(tcpsock_t));
    news->cookie = ((tcpsock_t*)s)->cookie; 
    news->port =  ((tcpsock_t*)s)->port;
    news->ip_addr = (char *)malloc(sizeof(char)*CHAR_IP_ADDR_LENGTH);
    news->ip_addr =strcpy(news->ip_addr, ((tcpsock_t*)s)->ip_addr);
    news->sd =  ((tcpsock_t*)s)->sd;
  return news;
}
...
{
  if(s1==s2){return 1;}
  else if(((tcpsock_t*)s1)->sd==((tcpsock_t*)s2)->sd){
    return 1;
  }
  else{return 0;}
  }
