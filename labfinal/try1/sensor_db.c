...
DBCONN * init_connection(char clear_up_flag){
	...
	printf("%s\n",sqlstate);
	// char * sqlstate ="DROP TABLE IF EXISTS "TO_STRING(TABLE_NAME)";"
	//           "CREATE TABLE "TO_STRING(TABLE_NAME)" (Id INTEGER PRIMARY KEY, sensor_id INT, sensor_value DECIMAL(4,2), timestamp TIMESTAMP);";
	                                                 
	// if(clear_up_flag!=1){
	// sqlstate ="CREATE TABLE "TO_STRING(TABLE_NAME)" (Id INTEGER PRIMARY KEY, sensor_id INT, sensor_value DECIMAL(4,2), timestamp TIMESTAMP);";
	// }
