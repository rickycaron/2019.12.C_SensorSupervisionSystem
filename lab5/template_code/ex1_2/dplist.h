...
typedef struct dplist  dplist_t; // slist_t is a struct containing at least a head pointer to the start of the list; later function pointers to element_copy, element_compare, element_free, etc. will be added. The implementation is in the c file but we just typedef it here for the function below to use it.
